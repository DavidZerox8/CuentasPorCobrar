<?php
	...
	if($id_cliente == "*"){ //Condición para identificar los usuarios elegidos (* es todos los clientes).
        
        $query = "select cp.id_cpc as cuenta, c.nombre_cliente as cliente, con.desc_concepto as concepto, con.tipo_concepto as tipoc, cp.n_factura as factura, cp.fecha_aplicacion as registro, cp.fecha_vencimiento as vencimiento, cp.monto as total, cp.estatus as estado, cp.observaciones as comentarios from cuentas_pc cp, clientes c, conceptos con where cp.n_concepto = con.id_concepto and cp.id_cliente = c.id_cliente and con.tipo_concepto != 'Abono'";

        $abonos_suma = "select sum(monto) as suma from abonos ";
        ...
        $pdf->Cell(70);
        $pdf->Cell(120,10, 'Reporte General de Movimientos',0,0,'C');
        $pdf->Ln(10);
		
            $pdf->SetFont('Arial','',12);
            $pdf->Cell(50,10, 'Cliente: Todos',0,1,'B');
            $pdf->Cell(50,10, 'Fecha de referencia: '.utf8_decode(date("d-m-Y")),0,1,'B');
            $pdf->Cell(50,10, 'Moneda: '.utf8_decode("Peso"),0,1,'B');
            $pdf->Ln(5);
        ...
        $query2 = "select sum(monto) as monto from cuentas_pc where estatus != 'Documentada'"; 
        ...
        if($count > 0){
            ...
            while($row = $resultado->fetch_assoc())
            {
                ...
                if($n_c != $n_cliente){
                    ...
                    $pdf->Cell(275,8,utf8_decode($n_cliente),1,1,'C',1);
                    $n_c = $n_cliente;
                }
            ...
            $pdf->Output('','Reporte global de Movimientos '.date("d-m-Y").'.pdf');
        
    }// Fin de la condición para todos los usuarios

    else{ //Inicio del proceso de reporte para un solo usuario
        ...
        if($count > 0){
            ...
            $pdf->Output('','Reporte de Movimientos '.date("d-m-Y").' - '.$nombre.'.pdf');
    }//Fin del else
