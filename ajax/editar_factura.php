		...
		$query_update = mysqli_query($con,$sql);
			if ($query_update){
               ...
               if($estado_factura == 1){
                   ...
                   $fechap=date("Y-m-d");
                   $updatecpc = "UPDATE `cuentas_por_cobrar` SET `fecha_pago` = '$fechap' WHERE `cuentas_por_cobrar`.`documento` = '$id_factura'"; 
                   
                   $query_new_insertcpc = mysqli_query($con,$updatecpc);
               }
                
                else if($estado_factura = 2)
                {
                    $updatecpc = "UPDATE `cuentas_por_cobrar` SET `fecha_pago` = 'Pendiente' WHERE `cuentas_por_cobrar`.`documento` = '$id_factura'"; 
